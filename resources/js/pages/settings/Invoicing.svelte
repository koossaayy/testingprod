<script module lang="ts">
    ...
    export const layout = {
        breadcrumbs: [
            {
                get title() { return get(_)('Invoicing settings'); },
                href: edit(),
            },
        ],
    };
...
<AppHead title={$_('Invoicing settings')} />

<h1 class="sr-only">{$_('Invoicing settings')}</h1>

<div class="flex flex-col space-y-6">
    <Heading
        variant="small"
        title={$_('Invoicing')}
        description={$_('Set the defaults we apply to every new invoice you raise')}
    />
    ...
    <form onsubmit={submit} class="space-y-6">
        <div class="grid gap-2">
            ...
            <Input
                ...
                required
                placeholder={$_('The name that appears at the top of your invoices')}
            />
            <InputError message={form.errors.business_name} />
        </div>

        <div class="grid gap-2">
            <Label for="invoice_prefix">{$_('Invoice number prefix')}</Label>
            <Input
                id="invoice_prefix"
            ...
            <p class="text-xs text-muted-foreground">
                {$_('Your next invoice will be numbered {0}.', { values: { 0: numberPreview } })}
            </p>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div class="grid gap-2">
                ...
                <Input
                    ...
                    title={$_('How many days a client gets to pay')}
                />
                <InputError message={form.errors.default_payment_terms_days} />
            </div>

            <div class="grid gap-2">
                <Label for="default_tax_rate">{$_('Default tax rate')}</Label>
                <Input
                    id="default_tax_rate"
                    type="number"
                    step="0.01"
        ...
        <div class="grid gap-2">
            <Label for="payment_instructions">{$_('Payment instructions')}</Label>
            <textarea
                ...
                placeholder={$_('Bank name, account number and any reference your client should quote')}
            ></textarea>
            <p class="text-xs text-muted-foreground">
                {$_('This block is printed at the bottom of every invoice. Don\'t include anything you would not want forwarded on.')}
            </p>
        ...
        <div class="flex items-start gap-3 rounded-lg border p-4">
            <Checkbox
                ...
                aria-label={$_('Send automatic payment reminders')}
            />
            <div class="grid gap-1">
                <Label for="send_payment_reminders">
                    {$_('Send automatic payment reminders')}
                </Label>
                <p class="text-xs text-muted-foreground">
                    {$_('We email the client three days before an invoice is due, and again the morning after it goes overdue.')}
                </p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4">
            <Button type="submit" disabled={form.processing}>
                {form.processing ? $_('Saving…') : $_('Save preferences')}
            </Button>
            {#if form.recentlySuccessful}
                <p class="text-sm text-muted-foreground">{$_('Saved.')}</p>
            {/if}
        </div>
    </form>
</div>
